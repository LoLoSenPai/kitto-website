...
const TokenInfo = ({ transactions, tokenBalances, mode }) => {
  ...
  return (
    <div className='w-full mx-auto md:p-5 md:max-w-6xl'>
      <div className='overflow-hidden'>
        {/* HEADER */}
        <div className={`grid ${gridClass} p-2 font-bold text-center text-sm sm:text-sm md:text-md xl:text-lg space-x-0 md:space-x-3`}>
          <div>Token</div>
        ...
        <div className='space-y-3'>
          {Object.keys(tokenData).map((token) => {
            ...
            return (
              <div key={token} className={`grid ${gridClass} p-2 text-center items-center	 bg-white rounded-lg shadow-lg bg-opacity-60 text-xs md:text-lg`}>
